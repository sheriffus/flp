% -*- mode: latex; mode: folding -*-
\documentclass[authoryear,9pt,preprint]{../../../gitroot/latex/sigplanconf}

%include lhs2TeX.fmt
%include lhs2TeX.sty

%% %include polycode.fmt
%include StateMonadLaws.fmt

%include header.tex


\begin{document}
%%%%%%%%%%%
\newcommand{\papertitle}{Getting to the |bottom| of |State|}

\conferenceinfo{Haskell'12,} {September 13, 2012, Copenhagen, Denmark.}
\CopyrightYear{2012}
%\copyrightdata{978-1-4503-0860-1/11/09} %TBD

\title{\papertitle}

\authorinfo{authors}%Patrik Jansson, Cl\'audio Amaral and M\'ario Florido}
{institutions}
%% {Chalmers University of Technology and \\University of Gothenburg \and
%% DCC - Faculty of Science, University of Porto \and
%% LIACC - University of Porto}
{emails}
%% {\{patrikj,amaral\}@@chalmers.se \and \{coa,amf\}@@ncc.up.pt}

\maketitle
\begin{abstract}
%include abstract.tex
\end{abstract}

\category{D.1.1}{Programming Techniques}{Applicative (Functional) Programming}
\category{F.3.3}{Logics and Meanings of Programs}{Studies of Program Constructs}[Type Structure]
% \category{F.4.1}{Mathematical Logic and Formal Languages}{Mathematical Logic}

% \terms{Design, Experimentation, Languages}

% \keywords
% Metaprogramming, Domain Specific Languages

%include sec1.tex
%include sec2.tex
%include sec3.tex
%include sec4.tex
%% %include sec5.tex
%% %include sec6.tex


%% \bibliographystyle{abbrvnat}
%% \bibliography{../../../gitroot/bibtex/jp,../../../gitroot/bibtex/genprog,../../../gitroot/bibtex/misc}


\end{document}
