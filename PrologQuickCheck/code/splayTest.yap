...
% {{{ splay
%% T1 = n(i1,v1,_A,_B) ? 
%% T2 = n(i2,v2,n(i1,v1,_A,_B),_C) ? 
%% T3 = n(i3,v3,n(i2,v2,n(i1,v1,_A,_B),_C),_D) ? 
%% T4 = n(i4,v4,n(i3,v3,n(i2,v2,n(i1,v1,_A,_B),_C),_D),_E) ? 
%%    ?- splay:splay_init(T), splay:splay_insert(i1, v1, T, T1), splay:splay_insert(i2, v2, T1, T2), splay:splay_insert(i3, v3, T2, T3), splay:splay_insert(i4, v4, T3, T4).
  % {{{ generator for splay commands

% we must create a generator of avl.yap uses (sequences of operations)
% this is parameterised by two disjoint generators that will make the elements for the tree and the elements for failing lookups
genSplay(KeyGen, ValGen, Calls, Size) :-
% it always starts with creating a tree
% this implies that when size is 0 the tree is still created
        Calls = [(splay:splay_init) | Cs1],
% and then is just a bunch of calls to insert and lookup values
        genSplayCmds(KeyGen, ValGen, [], Cs1, Size).

% generating random insertions and randomly choose some of those to lookup after, using difference lists for better append sublists
% zero size means zero calls
genSplayCmds(_KeyGen, _ValGen, _Lookups, [], 0) :- !. % cut - no other case if the size is 0
genSplayCmds(KeyGen, ValGen, Lookups, Cs, Size) :-
        genCmdHead(KeyGen, ValGen, Lookups, X, Size),
        %% call(KeyGen, Key, Size),
        (
            X = {i, {Key, Val}},
% commands are missing the continuations, these
%  will be controled by the property itself
            Cs = [ {i, Key, Val, splay:splay_insert} |CsS],
% update the possible correct lookups
            NewLookups = [ {Key,Val} | Lookups]
        ;
            ( X = {fl, {K, V}}, Lx=fl
            ...
            ; X = {l2, {K, V}}, Lx=l2),
% just put the lookup
            Cs = [ {Lx, K, V, splay:splay_lookup} |CsS]
        ;
% (d)elete and (f)ailed(d)elete - the latter doesn't actually fail
            ( X = {d, K}
            ; X = {fd, K}),
            Cs = [ {Lx, K, splay:splay_delete} |CsS]
        ;
            ( X = {j, L, R})
        ),
        MinSize is Size // 2,
        MaxSize is Size-1,
        choose(MinSize, MaxSize, NewSize, Size),
        genSplayCmds(KeyGen, ValGen, NewLookups, CsS, NewSize)
.

pair(G1, G2, X, Size) :- plqc:structure({G1, G2}, X, Size).

genCmdHead(KeyGen, ValGen, [], X, Size) :- !,
        pair(plqc:value(i), pair(KeyGen, ValGen), X, Size).
genCmdHead(KeyGen, ValGen, Lookups, X, Size) :- !,
        Insert = plqc:structure({plqc:value(i), pair(KeyGen, ValGen)}),
        Lookup1 = plqc:structure({plqc:value(l1), plqc:elements(Lookups)}),
        Lookup2 = plqc:structure({plqc:value(l2), plqc:elements(Lookups)}),
        Lookup3 = plqc:structure({plqc:value(fl), plqc:elements(Lookups)}),
        plqc:frequency([{6,Insert}, {2,Lookup1}, {2,Lookup2}, {1, Lookup3}], X, Size).
