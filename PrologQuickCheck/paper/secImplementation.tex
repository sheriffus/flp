...
of \Prolog{} code.  \todo{is this really a small program in the prolog
  comunity?}  \todo{should one even mention the thing we are trying to
...
predicate of the \plqc{} testing interface: \yap{quickcheck/[1,2]},
%% \yap{quickcheck/2}, \yap{quickcheckResult/2},
 \yap{quickcheckResult/[2,3]}.
%
The \yap{quickcheck/1} predicate tests a property and prints the results
...
\end{yapcode}
%
It is a default interface to \yap{quickcheck/2}, such that the second argument
...
The process of testing a property with \yap{quickcheckResult/3}
...
identification element (\yap{PredId}) and its corresponding
information (\yap{T}).
%
\begin{yapcode}
 user:term_expansion(
...
% vsc: how do you define the most important?
% coa: by some user criterion... just want to say that it can be used
% with the other form, but only one of the clauses can be like that
